...
/* Here we are using the GSAP (GreenSock Animation Platform) library to create scroll-triggered
animations on a webpage. */
...
/* Here we are creating a canvas element and setting its width and height to match the window's
inner width and height. It also adds an event listener to the window's resize event, so that
whenever the window is resized, the canvas is also resized and a render function is called. */
function canvas() {
 ...
 `;
    ...
    /**
     * Creating a scrolling animation effect on a canvas element,
     * where a sequence of images is displayed and scaled based on the scroll position.
     */
...
/* Creating a canvas element in the HTML document and setting its width and height to
match the window's inner width and height. It also adds an event listener to the window's resize
event, so that whenever the window is resized, the canvas is also resized and a render function is
called. */
function canvas1() {
...
`;
    ...
    /**
     * Createing a canvas animation that scrolls and scales a sequence of images
...
/* Using the GSAP (GreenSock Animation Platform) library to create animations
